...
bool CreateLedHSVTrajectoryViewActionHandler::Run() {
  switch(state_) {
    case kProcessingRequest: {
      const P2PCreateLedHSVTrajectoryViewRequest &request = GetRequest();
      ...
      char str[150];
      sprintf(str, "create_led_hsv_trajectory_view(id=%d, trajectory_id=%d, loop_after_seconds=%f, interpolation_config={type=%d})", trajectory_view_id, trajectory_id, loop_after_seconds, interpolation_config.type);
      LOG_INFO(str);
      ...
      if (maybe_trajectory_view.status() == Status::kDoesNotExistError) {
        ...
        const auto &maybe_trajectory = trajectory_store_.led_hsv_trajectories()[trajectory_id];
        if (!maybe_trajectory.ok()) {
          result_ = maybe_trajectory.status();
        } else {
          result_ = Status::kSuccess;
          maybe_trajectory_view = LedHSVTrajectoryView(&*maybe_trajectory);
...
bool CreateLedHSVTrajectoryViewActionHandler::TrySendingReply() {
  StatusOr<P2PActionPacketAdapter<P2PCreateLedHSVTrajectoryViewReply>> maybe_reply = NewReply();
  if (!maybe_reply.ok()) {
    return false;
  }
  P2PActionPacketAdapter<P2PCreateLedHSVTrajectoryViewReply> reply = *maybe_reply;
