...
bool CreateLedHSVMixedTrajectoryViewActionHandler::Run() {
  switch(state_) {
    case kProcessingRequest: {
      const P2PCreateLedHSVMixedTrajectoryViewRequest &request = GetRequest();
      ...
      if (maybe_mixed_trajectory_view.status() == Status::kDoesNotExistError) {
        ...
        const TrajectoryViewInterface<ColorHSVTargetState> *trajectory1_view = nullptr;
        const TrajectoryViewInterface<ColorHSVTargetState> *trajectory2_view = nullptr;
        const EnvelopeTrajectoryView *alpha_view = nullptr;

        switch(first_trajectory_view_type) {
          case P2PTrajectoryViewType::kPlain: {
            const auto &maybe_first_trajectory_view = trajectory_store_.led_hsv_trajectory_views()[first_trajectory_view_id];
          ...
          case P2PTrajectoryViewType::kModulated: {
            const auto &maybe_first_trajectory_view = trajectory_store_.led_hsv_modulated_trajectory_views()[first_trajectory_view_id];
            if (!maybe_first_trajectory_view.ok()) {
              result_ = maybe_first_trajectory_view.status();
          ...
          case P2PTrajectoryViewType::kMixed: {
            const auto &maybe_first_trajectory_view = trajectory_store_.led_hsv_mixed_trajectory_views()[first_trajectory_view_id];
            if (!maybe_first_trajectory_view.ok()) {
        ...
        if (result_ == Status::kSuccess) {
          switch(second_trajectory_view_type) {
            case P2PTrajectoryViewType::kPlain: {
              const auto &maybe_second_trajectory_view = trajectory_store_.led_hsv_trajectory_views()[second_trajectory_view_id];
            ...
            case P2PTrajectoryViewType::kModulated: {
              const auto &maybe_second_trajectory_view = trajectory_store_.led_hsv_modulated_trajectory_views()[second_trajectory_view_id];
              if (!maybe_second_trajectory_view.ok()) {
                result_ = maybe_second_trajectory_view.status();
            ...
            case P2PTrajectoryViewType::kMixed: {
              const auto &maybe_second_trajectory_view = trajectory_store_.led_hsv_mixed_trajectory_views()[second_trajectory_view_id];
              if (!maybe_second_trajectory_view.ok()) {
                result_ = maybe_second_trajectory_view.status();
        ...
        if (result_ == Status::kSuccess) {
          result_ = Status::kSuccess;
          maybe_mixed_trajectory_view = LedHSVMixedTrajectoryView();
...
bool CreateLedHSVMixedTrajectoryViewActionHandler::TrySendingReply() {
  StatusOr<P2PActionPacketAdapter<P2PCreateLedHSVMixedTrajectoryViewReply>> maybe_reply = NewReply();
  if (!maybe_reply.ok()) {
    return false;
  }
  P2PActionPacketAdapter<P2PCreateLedHSVMixedTrajectoryViewReply> reply = *maybe_reply;
