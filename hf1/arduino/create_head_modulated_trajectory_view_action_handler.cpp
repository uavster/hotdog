      ...
      if (maybe_modulated_trajectory_view.status() == Status::kDoesNotExistError) {
        ...
        if (result_ == Status::kSuccess) {
          switch(modulator_trajectory_view_type) {
            case P2PTrajectoryViewType::kPlain: {
              const auto &maybe_second_trajectory_view = trajectory_store_.head_trajectory_views()[modulator_trajectory_view_id];
            ...
            case P2PTrajectoryViewType::kModulated: {
              const auto &maybe_second_trajectory_view = trajectory_store_.head_modulated_trajectory_views()[modulator_trajectory_view_id];
              if (!maybe_second_trajectory_view.ok()) {
                result_ = maybe_second_trajectory_view.status();
            ...
            case P2PTrajectoryViewType::kMixed: {
              const auto &maybe_second_trajectory_view = trajectory_store_.head_mixed_trajectory_views()[modulator_trajectory_view_id];
              if (!maybe_second_trajectory_view.ok()) {
                result_ = maybe_second_trajectory_view.status();
