  ...
  switch(state_) {
    case kProcessingRequest: {
      ...
      if (maybe_modulated_trajectory_view.status() == Status::kDoesNotExistError) {
        ...
        const auto maybe_carrier_trajectory_view_id = trajectory_store_.head_trajectory_views()[carrier_trajectory_view_id];
        if (!maybe_carrier_trajectory_view_id.ok()) {
          result_ = maybe_carrier_trajectory_view_id.status();
        } else {
          const auto maybe_modulator_trajectory_view_id = trajectory_store_.head_trajectory_views()[modulator_trajectory_view_id];
