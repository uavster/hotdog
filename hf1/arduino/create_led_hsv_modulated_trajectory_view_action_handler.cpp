...
bool CreateLedHSVModulatedTrajectoryViewActionHandler::Run() {
  switch(state_) {
    case kProcessingRequest: {
      const P2PCreateLedHSVModulatedTrajectoryViewRequest &request = GetRequest();
      ...
      if (maybe_modulated_trajectory_view.status() == Status::kDoesNotExistError) {
        ...
        const TrajectoryViewInterface<ColorHSVTargetState> *carrier_view = nullptr;
        const TrajectoryViewInterface<ColorHSVTargetState> *modulator_view = nullptr;
        const EnvelopeTrajectoryView *envelope_view = nullptr;


        switch(carrier_trajectory_view_type) {
          case P2PTrajectoryViewType::kPlain: {
            const auto &maybe_carrier_trajectory_view = trajectory_store_.led_hsv_trajectory_views()[carrier_trajectory_view_id];
          ...
          case P2PTrajectoryViewType::kModulated: {
            const auto &maybe_carrier_trajectory_view = trajectory_store_.led_hsv_modulated_trajectory_views()[carrier_trajectory_view_id];
            if (!maybe_carrier_trajectory_view.ok()) {
              result_ = maybe_carrier_trajectory_view.status();
          ...
          case P2PTrajectoryViewType::kMixed: {
            const auto &maybe_carrier_trajectory_view = trajectory_store_.led_hsv_mixed_trajectory_views()[carrier_trajectory_view_id];
            if (!maybe_carrier_trajectory_view.ok()) {
              result_ = maybe_carrier_trajectory_view.status();
        ...
        if (result_ == Status::kSuccess) {
          switch(modulator_trajectory_view_type) {
            case P2PTrajectoryViewType::kPlain: {
              const auto &maybe_second_trajectory_view = trajectory_store_.led_hsv_trajectory_views()[modulator_trajectory_view_id];
            ...
            case P2PTrajectoryViewType::kModulated: {
              const auto &maybe_second_trajectory_view = trajectory_store_.led_hsv_modulated_trajectory_views()[modulator_trajectory_view_id];
              if (!maybe_second_trajectory_view.ok()) {
                result_ = maybe_second_trajectory_view.status();
            ...
            case P2PTrajectoryViewType::kMixed: {
              const auto &maybe_second_trajectory_view = trajectory_store_.led_hsv_mixed_trajectory_views()[modulator_trajectory_view_id];
              if (!maybe_second_trajectory_view.ok()) {
                result_ = maybe_second_trajectory_view.status();
        ...
        if (result_ == Status::kSuccess) {
          result_ = Status::kSuccess;
          maybe_modulated_trajectory_view = LedHSVModulatedTrajectoryView();
...
bool CreateLedHSVModulatedTrajectoryViewActionHandler::TrySendingReply() {
  StatusOr<P2PActionPacketAdapter<P2PCreateLedHSVModulatedTrajectoryViewReply>> maybe_reply = NewReply();
  if (!maybe_reply.ok()) {
    return false;
  }
  P2PActionPacketAdapter<P2PCreateLedHSVModulatedTrajectoryViewReply> reply = *maybe_reply;
