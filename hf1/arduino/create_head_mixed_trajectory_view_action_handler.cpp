  ...
  switch(state_) {
    case kProcessingRequest: {
      ...
      if (maybe_mixed_trajectory_view.status() == Status::kDoesNotExistError) {
        ...
        const auto &maybe_first_trajectory_view_id = trajectory_store_.head_trajectory_views()[first_trajectory_view_id];
        if (!maybe_first_trajectory_view_id.ok()) {
          result_ = maybe_first_trajectory_view_id.status();
        } else {
          const auto &maybe_second_trajectory_view_id = trajectory_store_.head_trajectory_views()[second_trajectory_view_id];
