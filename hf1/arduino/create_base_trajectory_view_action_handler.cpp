...
bool CreateBaseTrajectoryViewActionHandler::Run() {
  switch(state_) {
    case kProcessingRequest: {
      const P2PCreateBaseTrajectoryViewRequest &request = GetRequest();
      ...
      if (maybe_trajectory_view.status() == Status::kDoesNotExistError) {
        ...
        const auto maybe_trajectory = trajectory_store_.base_trajectories()[trajectory_id];
...
bool CreateBaseTrajectoryViewActionHandler::TrySendingReply() {
  StatusOr<P2PActionPacketAdapter<P2PCreateBaseTrajectoryViewReply>> maybe_reply = NewReply();
  if (!maybe_reply.ok()) {
    return false;
  }
  P2PActionPacketAdapter<P2PCreateBaseTrajectoryViewReply> reply = *maybe_reply;
